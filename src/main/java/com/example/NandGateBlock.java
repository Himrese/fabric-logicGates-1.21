...
public class NandGateBlock extends AbstractRedstoneGateBlock {
    public static final MapCodec<NandGateBlock> CODEC = createCodec(NandGateBlock::new);
    ...
    public static final Block AND_BLOCK = register("nand", new NandGateBlock(Block.Settings.create().strength(1.0f)));
    ...
    // 构造函数
    public NandGateBlock(AbstractBlock.Settings settings) {
        super(settings);
        this.setDefaultState(this.stateManager.getDefaultState().with(FACING, Direction.NORTH).with(LEFT_POWERED, false)
                .with(RIGHT_POWERED, false).with(POWERED, false).with(BOTTOM_POWERED, false));
    }
    ...
    protected boolean hasPower(World world, BlockPos pos, BlockState state) {
        ...
        boolean leftInput = world.getEmittedRedstonePower(pos.offset(facing.rotateYCounterclockwise()),
                facing.rotateYCounterclockwise()) > 0;
        boolean rightInput = world.getEmittedRedstonePower(pos.offset(facing.rotateYClockwise()),
