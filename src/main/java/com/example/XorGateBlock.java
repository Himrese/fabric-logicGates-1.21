...
public class XorGateBlock extends AbstractRedstoneGateBlock {
    public static final MapCodec<XorGateBlock> CODEC = createCodec(XorGateBlock::new);
    ...
    public static final Block AND_BLOCK = register("xor", new XorGateBlock(Block.Settings.create().strength(1.0f)));
    ...
    protected void appendProperties(StateManager.Builder<Block, BlockState> builder) {
        builder.add(FACING, POWERED, LEFT_POWERED, RIGHT_POWERED);
    }
    
    // 构造函数
    public XorGateBlock(AbstractBlock.Settings settings) {
        super(settings);
        this.setDefaultState(this.stateManager.getDefaultState().with(FACING, Direction.NORTH).with(LEFT_POWERED, false).with(RIGHT_POWERED, false).with(POWERED, false));
    }
    ...
    protected boolean hasPower(World world, BlockPos pos, BlockState state) {
        ...
        return leftInput ^ rightInput; // XOR逻辑
    }
